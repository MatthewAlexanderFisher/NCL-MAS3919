...
::: {.definition #def-manifold-hyp}
### Manifold hypothesis
The **manifold hypothesis** suggests that possibly high-dimensional data $\{\mathbf{x}_i\}_{i=1}^n \subset \mathbb{R}^d$ can be effectively represented in a lower-dimensional space, most often a \emph{smooth manifold} embedded in $\mathbb{R}^d$. 
...
Score matching and maximum likelihood estimation happen to yield the same result when training a normal generative model (\Cref{ex: sm and ml}), but in general these objectives can be different.
...
![Illustrating the denoising diffusion generative model. [[source](https://cvpr2023-tutorial-diffusion-models.github.io})]](figures/kernel_trick.png){#fig-denosing-diffusion width=70%}
