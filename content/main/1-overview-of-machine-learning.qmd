...
In practice the elements of $\mathcal{F}$ are usually parametrised, denoted $f_\theta : \mathcal{X} \rightarrow \mathcal{Z}$ for some *parameters* $\theta$ from a suitable index set $\Theta$.
...
::: {.definition #def-risk}
### Risk
Given a map $f : \mathcal{X} \rightarrow \mathcal{Z}$, we define the *risk* as
$$
...
### Empirical Risk
Given a map $f : \mathcal{X} \rightarrow \mathcal{Z}$, we define the *empirical risk* as
$$
